...
- 以后所有设计任务在未被特别说明时，默认采用**渐进式优化**，不做“秒变式”大改版，不突然重做整页视觉语言、布局骨架或信息架构。
- 当任务提供的是**移动端参考图**时，默认只允许重做移动端内容区，不得顺带重做桌面端布局。
- 桌面端在此类任务中必须保持原有信息架构、布局骨架和交互分区，除非用户明确要求“桌面端也一起改”。
- 若页面同时存在 header、导航栏、底部栏等全局共享区域，且用户说明“不要参考”或“不要修改”，则这些区域在所有端都不得改动。
- 若确需为移动端改版而调整桌面端样式，只允许做不改变布局结构的安全性修正，例如 spacing、bug fix、类型修复；不得新增桌面侧栏、改列数策略、重组模块层级。
- 在执行页面重设计前，必须先判断参考稿覆盖的是 `mobile`、`desktop` 还是 `both`；若不是 `both`，则另一端默认视为锁定状态。

### 搜索与顶部对齐规则

- 搜索相关页面或弹层中的搜索框，默认必须与全局 header 搜索框使用同一套高度、圆角、内边距和视觉样式；未明确说明时不得单独发明另一套搜索框规格。
- 所有功能组件的最高位置，不得超过 header 搜索框的上沿位置；header 搜索框的上沿是该页面内容区的默认顶部对齐基准。
- 若页面存在搜索历史、筛选、结果标题、快捷入口等功能模块，这些模块只能出现在 header 搜索框下方，不得顶到比搜索框更高的位置。
...
- **绝不提交到 Git 仓库**：任何服务端密钥、私钥、Admin SDK 凭证、数据库密码、第三方支付密钥、JWT Secret、SMTP 密码、Webhook Secret。
- **允许存在于客户端运行时但仍禁止入库的文件**：`android/app/google-services.json`、`GoogleService-Info.plist`、本地 `.env.local`、测试账号配置。
- **Firebase Android 配置规则**：
  - `android/app/google-services.json` 只允许保留在开发机和 CI/VPS 的私有文件系统中。
  - 必须加入 `.gitignore`，严禁再次提交到 public repo。
  - 即使其中的 Google API key 属于客户端 key，也必须在 Google Cloud Console 中配置限制，不能裸奔使用。
- **当前 TinyTale Firebase key 处理原则**：
  - 当前阶段允许继续使用现有 Firebase Android key。
  - 但必须至少加上 **Android app restriction**：包名 `top.tinytale.app` + 对应签名证书 SHA-1 / SHA-256。
  - 同时加上 **API restrictions**，仅保留 Firebase 所需 API。
- **环境变量分层**：
  - 前端公开变量只能使用 `NEXT_PUBLIC_*`，且只放可公开信息。
  - 后端私密变量只能放在 API 服务端环境，不得通过 `NEXT_PUBLIC_*`、JSON 配置文件或浏览器可读接口暴露。
  - VPS 部署时通过服务器本地环境文件或 PM2 环境注入，不写死在仓库代码里。
- **提交前检查**：
  - 提交前必须搜索一次敏感信息：API key、private key、service account、`google-services.json`、`.env`。
  - 若发现疑似密钥，先移出版本控制，再提交。
- **事故处理**：
  - 一旦发现 key 已进入 Git 历史，先从当前分支删除并停止继续传播。
  - 若仓库为公开仓库，需评估是否改写 Git 历史并强推清理。
