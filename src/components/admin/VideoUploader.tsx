...
function getPreferredChunkSize(fileSize: number): number {
...
function getRetryDelays(): number[] {
  if (typeof window === 'undefined') {
    return [0, 1000, 3000, 7000, 15000, 30000];
  }
  const effectiveType = (navigator as any)?.connection?.effectiveType as string | undefined;
  const isSlowNetwork = effectiveType === 'slow-2g' || effectiveType === '2g' || effectiveType === '3g';
...
function getTusErrorMessage(error: unknown): string {
  const fallback = 'Upload failed. Please check your network and retry.';
...
export default function VideoUploader({
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      const upload = new tus.Upload(file, {
        ...
        chunkSize: getPreferredChunkSize(file.size),
        ...
        onShouldRetry: (err, retryAttempt, options) => {
          ...
          if (typeof status === 'number' && status >= 400 && status < 500 && ![408, 409, 425, 429].includes(status)) {
            return false;
          }
        ...
        onError: (err) => {
          const msg = getTusErrorMessage(err);
          setErrorMsg(msg);
          setState('error');
          onError?.(msg);
        },
  ...
  }, [onUploadComplete, onUploadSessionCreated, onProgress, onError, uploadSessionId]);
