...
function StepVideoSubtitles({
  ...
  const monitorUploadedVideoReady = useCallback(async (episodeId: string, videoUid: string) => {
    ...
    setForm((prev) => ({
      ...
      episodes: prev.episodes.map((episode) => (
        episode.id === episodeId
          ? { ...episode, description: "Video processing timeout. Please retry upload.", status: "processing" }
          : episode
      )),
    }));
  }, [API_BASE, setForm]);
    ...
    (async () => {
      try {
        ...
        const upload = new tus.Upload(file, {
          ...
          onError: (err) => {
            ...
            setForm((prev) => ({
              ...prev,
              episodes: prev.episodes.map((ep) => (
                ep.id === epId ? { ...ep, status: "processing", uploadProgress: 0, description: `Upload failed: ${err.message}` } : ep
              )),
            }));
            setUploadQueue((q) => q.slice(1));
            setIsUploading(false);
        ...
        setForm((prev) => ({
          ...prev,
          episodes: prev.episodes.map((ep) => (
            ep.id === epId ? { ...ep, status: "processing", uploadProgress: 0, description: `Upload failed: ${err.message || "Network error"}` } : ep
          )),
        }));
        setUploadQueue((q) => q.slice(1));
        setIsUploading(false);
  ...
  const pollClipStatus = (clipUids: string[]) => {
    ...
    clipPollTimeoutRef.current = setTimeout(() => clearClipPolling(), 600000);
  };
      ...
      <div className="rounded-xl bg-[#1a1a2e] p-6">
        ...
        <div className="mb-5 flex items-center gap-3">
          ...
          {form.episodes.length > 0 && (
            <span className={`ml-auto rounded px-2.5 py-1 text-xs font-medium ${allEpisodesReady ? "bg-green-500/20 text-green-400" : "bg-amber-500/20 text-amber-400"}`}>
              {allEpisodesReady ? "All episodes ready" : "Waiting for processing"}
            </span>
          )}
        </div>
