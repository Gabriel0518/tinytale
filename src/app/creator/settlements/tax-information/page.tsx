...
export default function CreatorSettlementTaxInformationPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setForm(response.data);
      toast(t("Tax information saved."), "success");
    } catch (error) {
      toast(error instanceof Error ? error.message : t("Failed to save tax information."), "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6 xl:space-y-7">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-[15px] font-semibold text-[#64748b]">{t("Settlement")}</p>
          <h1 className="text-[34px] font-black tracking-[-0.04em] text-[#0f172a] md:text-[40px]">{t("Tax Information / W-9")}</h1>
        </div>
      </div>
      ...
      <section className="rounded-[28px] bg-[linear-gradient(135deg,#edf5ff,#f8fbff)] px-6 py-6 shadow-[0_1px_2px_rgba(15,23,42,0.05)] md:px-8 md:py-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-[760px]">
            <p className="text-[14px] font-semibold uppercase tracking-[0.08em] text-[#64748b]">{t("Compliance Profile")}</p>
            <p className="mt-3 text-[28px] font-black tracking-[-0.04em] text-[#0f172a]">{t("Submit the tax profile used for creator settlement and annual statements.")}</p>
            <p className="mt-3 text-[16px] leading-7 text-[#64748b]">
              {t("This information is used for settlement review, payout reconciliation, and PDF earnings statements.")}
            </p>
          </div>
          <div className="grid min-w-[280px] gap-3 rounded-[24px] border border-[#dbe7f6] bg-white px-5 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[13px] font-semibold uppercase tracking-[0.08em] text-[#94a3b8]">{t("Current Status")}</p>
                <p className="mt-1 text-[18px] font-bold text-[#0f172a]">{form.status === "submitted" ? t("Submitted") : t("Missing")}</p>
              </div>
            </div>
            <div className="rounded-[18px] bg-[#f8fafc] px-4 py-3 text-[14px] text-[#64748b]">
              {t("Last updated")}: <span className="font-semibold text-[#0f172a]">{formatDate(form.updatedAt, locale, t)}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <form className={`${cardClassName} p-6 md:p-7`} onSubmit={handleSubmit}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-[22px] font-bold tracking-[-0.02em] text-[#0f172a]">{t("W-9 Details")}</h2>
            <p className="mt-2 text-[15px] leading-7 text-[#64748b]">
              {t("Enter the name, taxpayer identification number, and mailing address that should appear on your settlement documentation.")}
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <div className="grid grid-cols-[140px_minmax(0,1fr)] gap-3">
            <div>
              <FieldLabel>{t("Tax ID Type")}</FieldLabel>
              <select
                className={inputClassName}
            ...
            <div>
              <FieldLabel>{t("Tax ID Number")}</FieldLabel>
          ...
          <div>
            <FieldLabel>{t("State / Region")}</FieldLabel>
          ...
          <div className="md:col-span-2">
            <FieldLabel>{t("Certification Signature")}</FieldLabel>
            ...
            <p className="mt-2 text-[13px] leading-6 text-[#94a3b8]">
              {t("By signing, you certify that the taxpayer information provided is accurate and may be used on settlement statements.")}
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-end gap-3 border-t border-[#edf2f7] pt-5">
          ...
          >
            {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : t("Save W-9 Information")}
          </button>
        </div>
      </form>
    </div>
