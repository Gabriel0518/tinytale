...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    ...
    icon: Wallet,
    matcher: (path) => path === "/creator/analytics/revenue" || path.startsWith("/creator/settlements"),
  },
  {
    label: "Settings",
...
export default function CreatorSidebar({ locale, normalizedPath, user }: CreatorSidebarProps) {
  ...
  return (
    <aside className="fixed left-0 top-0 z-20 hidden h-screen w-[288px] flex-col justify-between border-r border-[#e2e8f0] bg-white lg:flex">
      <div>
        <div className="flex items-center gap-3 p-6">
          <div className="flex h-8 w-9 items-center justify-center rounded-2xl bg-[#1876f2]">
            <BookMarked className="h-5 w-5 text-white" />
          </div>
          <div>
            <p className="text-[20px] font-black leading-5 tracking-[-0.5px] text-[#0f172a]">TinyTale</p>
            <p className="mt-1 text-xs font-medium uppercase tracking-[0.06em] text-[#64748b]">Creator Hub</p>
          </div>
        </div>
          ...
          {NAV_ITEMS.map(({ label, href, icon: Icon, matcher }) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-base leading-6 ${active ? "font-semibold" : "font-medium"}`}>{label}</span>
              </Link>
            );
          })}
        </nav>
      ...
      <div className="border-t border-[#e2e8f0] px-4 py-4">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs leading-4 text-[#64748b]">Pro Creator</p>
          </div>
