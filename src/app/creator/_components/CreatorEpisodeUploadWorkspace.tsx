...
export default function CreatorEpisodeUploadWorkspace({ initialDramaId }: CreatorEpisodeUploadWorkspaceProps) {
  ...
  const startEpisodeVideoUpload = useCallback(
    async (episode: CreatorEpisodeItem, file: File, hooks?: VideoUploadHooks): Promise<boolean> => {
      ...
      try {
        const initRes = await creatorApi.requestVideoUpload(token, {
          filename: file.name,
          filesize: file.size,
        ...
        const { upload_url, video_uid } = initRes.data;
        activeVideoUidRef.current[episode._id] = video_uid;

        await patchEpisode(episode._id, {
          streamVideoId: video_uid,
        ...
        await new Promise<void>((resolve, reject) => {
          const upload = new tus.Upload(file, {
            ...
            chunkSize: file.size > 512 * 1024 * 1024 ? 20 * 1024 * 1024 : 8 * 1024 * 1024,
            retryDelays: [0, 1000, 3000, 7000, 15000, 30000],
        ...
        const ready = await pollVideoStatus(episode, video_uid);
        if (ready) {
          hooks?.onDone?.();
        } else {
    ...
    async (file: File) => {
      ...
      try {
        ...
        for (let attempt = 0; attempt < 24; attempt += 1) {
          const status = await creatorApi.getVideoUploadStatus(token, video_uid);
