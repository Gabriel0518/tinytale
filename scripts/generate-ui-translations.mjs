...
const OUTPUT_FILE = path.join(ROOT, 'src/lib/ui-translations.generated.json');
const TARGET_LOCALES = ['ko', 'fr'];
...
function listSourceFiles() {
  const command = [
    'rg -l',
    '"SupportedLocale|\\[locale\\]\\s*\\|\\||EN_COPY|COPY_EN|DATE_LOCALE_MAP|_TEXT\\s*:\\s*|COPY\\s*:\\s*"',
...
function shouldKeepPhrase(phrase) {
  ...
  if (/^[a-z0-9-:./_%#()[\]]+$/i.test(text) && (text.includes('/') || text.includes('-') || text.includes(':'))) return false;
  if (/^[a-z0-9_]+$/.test(text) && text === text.toLowerCase()) return false;
  if (/^[MLHVCSQTAZmlhvcsqtaz0-9.,\s-]+$/.test(text)) return false;
  if (text.length > 32 && !text.includes(' ')) return false;
  const letterCount = (text.match(/[A-Za-z]/g) || []).length;
  if (letterCount / text.length < 0.45) return false;
...
async function main() {
  ...
  console.log(`Missing translations: ${tasks.length}`);
  if (DRY_RUN) return;
